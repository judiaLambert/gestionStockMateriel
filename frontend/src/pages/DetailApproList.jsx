...
import { Plus, Search, Edit, Trash2, Truck, Package, Calendar, Download, Filter, TrendingUp, Eye, X } from 'lucide-react';
...
const ApprovisionnementList = () => {
  ...
  const handleDeleteDetail = async (detail) => {
    if (confirm(`Supprimer le matériel "${detail.materiel?.designation}" ?`)) {
      try {
  ...
  const handleSubmitDetail = async (e) => {
    ...
    try {
      ...
      setDetailFormData({ id: '', id_materiel: '', id_appro: '', quantite_recu: '', prix_unitaire: '' });
      fetchData();
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      ...
      {/* Modal Détails avec gestion complète */}
