...
const TypeMaterielList = () => {
  ...
  const handleDelete = async (row) => {
    if (confirm(`Supprimer le type "${row.designation}" ?`)) {
      try {
        await deleteTypeMateriel(row.id);
        fetchTypes();
      } catch (err) {
        console.error(err);
      }
  ...
  const headers = ['ID', 'Désignation', 'Description'];

  const tableData = typesMateriel.map(tm => ({
    id: tm.id,
    designation: tm.designation,
    description: tm.description || 'Aucune description'
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-800">Gestion des Types de Matériel</h1>
        <button onClick={() => setIsModalOpen(true)} className="bg-green-600 text-white px-6 py-3 rounded-lg flex items-center gap-2">
      ...
      <Modal
        ...
        title={isEditing ? "Modifier Type de Matériel" : "Ajouter Type de Matériel"}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2 border rounded-lg"
              placeholder="Description détaillée du type de matériel"
              rows="4"
            />
          </div>
